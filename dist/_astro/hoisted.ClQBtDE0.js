import{c as X,a as A,b as C,d as c,e as U,f as V,h as G,i as J}from"./ViewTransitions.astro_astro_type_script_index_0_lang.DxLj3x3s.js";import"https://cdn.jsdelivr.net/npm/chart.js@4.4.4/dist/chart.umd.min.js";import"./Toast.astro_astro_type_script_index_0_lang.BMihgS09.js";function K(){const d=X(),m=document.getElementById("dashboard-loading"),p=document.getElementById("dashboard-content");if(!d){m.textContent="",p.classList.remove("hidden");return}m.remove(),p.classList.remove("hidden");const s=new Date,o=s.toISOString().slice(0,10),a=A(),u=C(o),B=c(o),g=s.getHours(),I=g<12?"Buenos días":g<18?"Buenas tardes":"Buenas noches";document.getElementById("hero-greeting").textContent=`${I}, ${d.name}`;const x=new Intl.DateTimeFormat("es-MX",{weekday:"long",year:"numeric",month:"long",day:"numeric"}).format(s);document.getElementById("hero-date").textContent=x.charAt(0).toUpperCase()+x.slice(1);const b=[[4,"Base"],[8,"Construcción"],[12,"Intensificación"],[16,"Peak & Taper"]].find(([e])=>a<=e)?.[1]||"Peak & Taper",D=document.getElementById("hero-badge-row");D.innerHTML=`
      <span class="text-xs bg-surface2 text-muted px-3 py-1 rounded-full">Semana ${a} · Fase ${b}</span>
    `;const M={fuerza:"#e8ff47",cardio:"#47c2ff",mixto:"#ff6b35",descanso:"#8890a8",voleibol:"#47c2ff"},S=document.getElementById("session-type-row"),L=M[u.type]||"#8890a8";S.innerHTML=`
      <span class="font-display text-3xl" style="color:${L}">${u.label.toUpperCase()}</span>
      ${B?.completed?'<span class="text-green-400 text-sm flex items-center gap-1"><svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>Completada</span>':""}
    ...
    `;const T=U(),f=V(),y=G(),l=c(o)?.water??null,E=document.getElementById("metrics-grid"),H=[{label:"Racha actual",value:T.toString(),unit:"días"},{label:"Esta semana",value:`${f.completed} / ${f.total}`,unit:"sesiones"},{label:"Último ritmo 5K",value:y||"—",unit:y?"min/km":""},{label:"Agua hoy",value:l!==null?l.toFixed(1):"—",unit:l!==null?"L":""}];E.innerHTML=H.map(e=>`
    ...
    `).join("");const F=document.getElementById("week-pills"),r=new Date(s),h=s.getDay(),j=h===0?-6:1-h;r.setDate(s.getDate()+j);const w=[];for(let e=0;e<7;e++){const n=new Date(r);n.setDate(r.getDate()+e);const t=n.toISOString().slice(0,10),k=c(t),O=C(t),i=t===o,P=["D","L","M","X","J","V","S"][n.getDay()],z=n.getDate(),N={fuerza:"bg-yellow",cardio:"bg-blue",mixto:"bg-orange",descanso:"bg-muted",voleibol:"bg-blue"}[k?.type||O?.type]||"bg-muted2",R=i?"border-2 border-yellow bg-yellow/5":"border border-[rgba(255,255,255,0.07)] bg-[#161920]",W=k?.completed?'<svg class="w-3.5 h-3.5 text-green-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>':`<span class="w-2 h-2 rounded-full ${N}"></span>`;w.push(`
        <a href="/day/?date=${t}" class="flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all min-w-[52px] flex-shrink-0 ${R}">
          <span class="text-[10px] uppercase tracking-wider ${i?"text-yellow":"text-muted"}">${P}</span>
